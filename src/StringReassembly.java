...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String combo = "";
        String str1Check = str1.substring(str1.length() - overlap);
        //needs to be length of overlap starting from the right handside
        String str2Check = str2.substring(0, overlap);
        if (str1Check.equals(str2Check)) {
            combo = str1.substring(0, str1.length() - overlap) + str2;
        }
        return combo;
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        if (!(strSet.size() == 0)) {
            Set<String> removeable = new Set1L<String>();
            for (String s : strSet) {
                if (str.indexOf(s) != -1) {
                    removeable.add(s);
                }
            }
            strSet.remove(removeable);
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        String strToAdd = input.nextLine();
        while (!(strToAdd.equals(""))) {
            addToSetAvoidingSubstrings(inputSet, strToAdd);
            strToAdd = input.nextLine();
        }
        return inputSet;
    }
